...
module "ecs-task-definition" {
    source = "github.com/pbs/terraform-ecs-task-definition-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/ecs-task-definition'
gh -R 'pbs/terraform-ecs-task-definition-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/ecs-task-definition'
rm -f release.tar.gz
```
...
```bash
git remote add -f ecs-task-definition git@github.com:pbs/terraform-ecs-task-definition-module.git
git subtree add --prefix terraform/modules/ecs-task-definition ecs-task-definition main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/ecs-task-definition ecs-task-definition $REF --squash
```
